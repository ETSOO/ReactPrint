...
export default class ReactToPrint extends React.Component<IReactToPrintProps> {
    ...
    public handlePrint = (callSize?: PageSize) => {
        ...
        printWindow.height = `${height}px`;
        printWindow.style.position = 'absolute';
        printWindow.style.top = `-${clientHeight + 100}px`;
        printWindow.style.left = `-${clientWidth + 100}px`;
        ...
        printWindow.onload = () => {
            ...
            if (domDoc) {
                ...
                if (fonts) {
                    const docFonts = printWindow.contentDocument?.fonts;
                    if (docFonts && typeof FontFace != undefined) {
                        fonts.forEach((font) => {
                            const fontFace = new FontFace(
                                font.family,
                                font.source
                            );
                            docFonts.add(fontFace);
                            fontFace.loaded
                                .then((loadedFontFace) => {
                ...
                if (typeof defaultPageStyle !== 'string') {
                    ...
                    styleEl.appendChild(
                        domDoc.createTextNode(defaultPageStyle)
                    );
                ...
                if (!isText) {
                    ...
                    const srcCanvasEls = isText
                        ? []
                        : (contentNodes as Element).querySelectorAll('canvas');
                    ...
                    for (let i = 0; i < renderComponentVideoNodes.length; i++) {
                        ...
                        if (videoPoster) {
                            ...
                            if (videoNode.readyState >= 2) {
                                ...
                                videoNode.onloadeddata = markLoaded.bind(
                                    null,
                                    videoNode,
                                    true
                                );
                    ...
                    const originalCRs = (
                        contentNodes as HTMLElement
                    ).querySelectorAll(checkedSelector);
                    ...
                    const copiedSelects =
                        domDoc.querySelectorAll(selectSelector);
                ...
                if (copyStyles) {
                    ...
                    for (
                        let i = 0, headElsLen = headEls.length;
                        i < headElsLen;
                        ++i
                    ) {
                        ...
                        if (node.tagName.toLowerCase() === 'style') {
                            // <style> nodes
                            const newHeadEl = domDoc.createElement(
                                node.tagName
                            );
                            ...
                            if (sheet) {
                                ...
                                try {
                                    ...
                                    for (let j = 0; j < cssLength; ++j) {
                                        if (
                                            typeof sheet.cssRules[j].cssText ===
                                            'string'
                                        ) {
                                ...
                                newHeadEl.appendChild(
                                    domDoc.createTextNode(styleCSS)
                                );
                            ...
                            if (node.getAttribute('href')) {
                                ...
                                if (!node.hasAttribute('disabled')) {
                                    ...
                                    newHeadEl.onload = markLoaded.bind(
                                        null,
                                        newHeadEl,
                                        true
                                    );
                                    newHeadEl.onerror = markLoaded.bind(
                                        null,
                                        newHeadEl,
                                        false
                                    );
